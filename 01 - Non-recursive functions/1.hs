...
addMinutes h c =
    (mod (div ((convertToMins h) + c) 60) 24, mod ((convertToMins h) + c) 60)
...
addMinutes1 a x =
    (H (mod (div ((convertToMins1 a) + x) 60) 24)
         (mod ((convertToMins1 a) + x) 60))
...
vertices (Circle _ _) = []
vertices (Rectangle p1 p2) =
    [p1, Cartesian (posx p1) (posy p2), p2, Cartesian (posx p2) (posy p1)]
...
perimeter2 (Triangle p1 p2 p3) =
    dist2points p1 p2 + dist2points p2 p3 + dist2points p3 p1
perimeter2 (Rectangle p1 p2) =
    2 * (abs (posx p2 - posx p1) + abs (posy p2 - posy p1))
