...
sumTriple [] = (0, 0, 0)
sumTriple ((x, y, z):t) =
    (x + fst3 (sumTriple t), y + snd3 (sumTriple t), z + thd3 (sumTriple t))
...
respectiveInt [] = []
respectiveInt (h:t)
    | ord h >= 49 && ord h <= 57 = ((ord h) - 48) : (respectiveInt t)
...
deriv [] = []
deriv (h:t)
    | snd h /= 0 = (((fst h) * fromIntegral ((snd h))), (snd h) - 1) : (deriv t)
    | otherwise = deriv t
...
normalizePoly (h:t)
    | (snd h) == (snd (head t)) =
        (fst h + fst (head t), snd h) : (normalizePoly (tail t))
...
addPoly [] b = b
addPoly a b
    | (normalizePoly a == a) || (normalizePoly b == b) =
        normalizePoly ((head a) : (addPoly (tail a) b))
