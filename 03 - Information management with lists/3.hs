...
checkTrip (h:t) =
    ((checkStep h && checkTrip t) && (checkStep ((snd h), (fst (head t)))))
...
polygonalLength [_] = 0
polygonalLength (h:t) =
    (dist2points h (head t)) + (polygonalLength ((head t) : (tail t)))
...
createTriangles _ [] = []
createTriangles _ [_] = []
createTriangles p1 (p2:p3:ps) =
    (Triangle p1 p2 p3) : (createTriangles p1 (p3 : ps))
...
polygonalArea l =
    abs (sum [ x1 * y2 - x2 * y1
             | (p1, p2) <- zip l (tail (cycle l))
             , let (x1, y1) = getCartesian p1
             , let (x2, y2) = getCartesian p2
             ])
        / 2
...
data Statement =
    Stmt Float [(Date, String, Operation)]
    deriving (Show)
