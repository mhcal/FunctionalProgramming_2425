...
data FTree a b = Leaf b | Node a (FTree a b) (FTree a b) -- you can have separate types for leaves and nodes
               deriving Show
data BTree a = Empty | BNode a (BTree a) (BTree a)-- same type as from the last exercise sheet; changed node constructor name to avoid confusion with ftree's node constructor
             deriving Show
...
splitFTree (Node n l r) =
  (BNode n (fst $ splitFTree l) (fst $ splitFTree r), Fork (snd $ splitFTree l) (snd $ splitFTree r))
