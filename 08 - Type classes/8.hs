...
instance Show IntExp -- don't need additional constraints since regular Int is already an instance of Show
                                                                                                           where
    show (Const a) = show a
      ...
      where
        ...
        (crl, dbl) =
            let isCredit (Credit _) = True
                ...
                getcrl =
                    maximum $ map (length . show . (\(Credit x) -> x)) credl
                getdbl = maximum $ map (length . show . (\(Debit x) -> x)) debl
             in (max 6 getcrl, max 5 getdbl)
        d =
            let divisorlength =
                    sum [max 4 dl, 2, max 11 sl, 2, max 6 crl, 2, max 5 dbl]
