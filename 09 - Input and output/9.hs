...
data Bet = B [Int] (Int, Int) deriving Show
...
validate (B n (s1, s2)) =
    (s1 /= s2) && (min s1 s2 >= 1) &&
    (max s1 s2 <= 9) && (length n == 5) &&
    foldr ((&&) . (\x -> x <= 50 && x >= 1 && not (elem x (delete x n)))) True n
...
readBet = do
    ...
    if validate bet then return bet
    else do putStrLn $ "> Invalid bet. Try again." 
            readBet
